...
run()
{
    ...
    # Run without 'exec' to keep as child process
    unshare --fork --pid --uts --net --ipc --mount chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c " chmod 755 / && mount -t proc proc /proc && mount -t sysfs sysfs /sys && $INIT_CMD_ARGS "
...
exec()
{
    ...
    # running within the container
     nsenter --target $CONTAINER_INIT_PID --uts --net --pid --ipc --mount -w -r $EXEC_CMD_ARGS
...
addnetwork()
{
    ...
    ip -n $NAME link set $INSIDE_PEER netns $NAME up
